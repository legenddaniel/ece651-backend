        ...
        tests = [
            {
                ...
                'wait': lambda: WebDriverWait(self.browser, 5).until(
                    lambda: form.find_element(by=By.CSS_SELECTOR, value='.fail')),
                'assertions': [
                    # Existing user
            ...
            },
            {
                ...
                'wait': lambda: WebDriverWait(self.browser, 5).until(
                    lambda: form.find_element(by=By.CSS_SELECTOR, value='.succ > a.login')),
                'assertions': [
                    # Valid email
                    lambda: self.assertEqual(user.username, USERNAME),
                ]
            },
        ]
        for test in tests:
